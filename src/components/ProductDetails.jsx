...
const ProductDetails = () => {
  ...
  async function handleAddToCart(){
    ...
    if(isAuth){
      let resp = await axios.put(`${domain}/api/v1/cart`,{
        type : 'add',
        productId : prod._id,
        quantity : 1
  ...
  async function fetchRecomandations(){
    let resp = await axios.get(`${domain}/api/v1/products?city=${ prod.city }`,{ withCredentials: true });
  ...
  async function fetchCategoryReccomandations(){
    if(prod.category){
      const resp = await axios.get(`${domain}/api/v1/products?category=${ prod.category }`,{ withCredentials: true });
      setprodCategories( resp.data.products );
    }
  }

  async function getProduct(){
    const resp = await axios.get(`${domain}/api/v1/product/${params.id}`,{ withCredentials: true });
